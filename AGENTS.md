...
- **Kids video image provider (DEFAULT + REQUIRED):** **Qwen-Image (`runware:108@1`) via Runware** is the default and required image-generation model for the kids video system going forward. All future kids-video scene/image generation MUST use this model through Runware unless explicitly overridden by a future instruction. Do not use the previous kids image-generation model (FLUX.2 [klein] 9B) as the default — it has been superseded. Character consistency is achieved via a detailed, locked-in character description repeated verbatim in every scene prompt + a fixed seed (pure text-to-image, no reference/seed image). The kids image pipeline is text-safe: it reads each scene's selected Remotion component's text-placement/safe-area config and instructs the image model to keep characters and important objects out of the reserved text region so captions never overlap them.
