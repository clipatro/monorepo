...
import { loadEnv, spikeDir, writeArtifact, writeBinaryArtifact, type SpikeResult } from "./lib/spike.ts";
...
export interface MediaArtifact {
  images: Array<{ sceneId: string; path: string; width: number; height: number; costUsd: number }>;
  totalCostUsd: number;
}
...
async function runResearch(
  ...
  if (isDryRun()) {
    ...
    await writeArtifact("s21", "01-research.json", JSON.stringify(artifact, null, 2));
    log("Research", `DRY-RUN: ${artifact.sources.length} sources, ${artifact.claims.length} claims, ${artifact.allowedFacts.length} facts — $0.0000`);
    return artifact;
  }
...
Do not fabricate sources, URLs, or statistics. Use only verified information.`,
    useGrounding: true,
    systemInstruction: "You are an evidence-first financial researcher. Find real data and authoritative sources. Never fabricate statistics, dates, or quotations.",
    temperature: 0.2,
  ...
  await writeArtifact("s21", "01-research.json", JSON.stringify(artifact, null, 2));
  log("Research", `${sources.length} sources, ${claims.length} claims, ${artifact.allowedFacts.length} facts — ${costSummary(artifact.costUsd)}`);
  return artifact;
}
...
async function runScript(
  ...
  if (isDryRun()) {
    ...
    await writeArtifact("s21", "02-script.json", JSON.stringify(artifact, null, 2));
  ...
  const catalogCompact = catalog.components.map((c) => ({
    ...
    tones: c.tones,
    media: c.media,
    inputs: c.inputs.map((i) => ({ name: i.name, kind: i.kind, required: i.required, max: i.maxCharacters ?? i.maxItems })),
...
Return JSON in this exact format:
{
  ...
  const result = await client.call({
    ...
    prompt,
    responseJson: true,
    systemInstruction: "You are a master scriptwriter for short-form financial documentaries. You write clear, engaging, spoken-word narration. You select the best visual component for each beat. Return ONLY valid JSON.",
    temperature: 0.7,
...
async function runScenePlan(
  ...
  await writeArtifact("s21", "03-scene-plan.json", JSON.stringify(artifact, null, 2));
...
async function runMedia(
  ...
  const imageScenes = script.scenes.filter((s) => s.needsImage && s.imagePrompt);

  if (imageScenes.length === 0) {
    log("Media", "No scenes require images — skipping");
    const artifact: MediaArtifact = { images, totalCostUsd: 0 };
    await writeArtifact("s21", "04-media.json", JSON.stringify(artifact, null, 2));
    return artifact;
  }
  ...
  for (const scene of imageScenes) {
    ...
    if (isDryRun()) {
      ...
      if (!res.ok) throw new Error(`Image generation failed for ${scene.id}: ${raw.error?.message ?? res.status}`);

      const imagePart = raw.candidates?.[0]?.content?.parts?.find((p: any) => p.inlineData?.data);
      if (!imagePart?.inlineData?.data) throw new Error(`No image returned for ${scene.id}`);
    ...
    images.push({ sceneId: scene.id, path: imgPath, width: WIDTH, height: HEIGHT, costUsd });
  }
...
${narration}
  ...
  await writeArtifact("s21", "05-narration.json", JSON.stringify(artifact, null, 2));
  return artifact;
}
...
async function runMusicSync(
  ...
  if (!await exists(BACKGROUND_MUSIC)) {
    ...
    const artifact: MusicSyncArtifact = { mixedAudioPath: mixedPath, durationSec: narrationDurationSec, costUsd: 0 };
    await writeArtifact("s21", "06-music-sync.json", JSON.stringify(artifact, null, 2));
    return artifact;
  }
  ...
  log("MusicSync", `Mixed audio ${durationSec.toFixed(1)}s (narration + background music)`);
  const artifact: MusicSyncArtifact = { mixedAudioPath: mixedPath, durationSec, costUsd: 0 };
  await writeArtifact("s21", "06-music-sync.json", JSON.stringify(artifact, null, 2));
  return artifact;
}
...
async function runComposition(
  ...
  log("Composition", `Public: ${publicDir} (${media.images.length} images + audio)`);
  ...
  await writeArtifact("s21", "07-composition.json", JSON.stringify(artifact, null, 2));
  return artifact;
}
...
async function runRender(
  ...
  const artifact: RenderArtifact = { videoPath, durationSec, sizeBytes, costUsd: 0 };
  await writeArtifact("s21", "08-render.json", JSON.stringify(artifact, null, 2));
  return artifact;
}
...
function generateMockResearch(topic: string): ResearchArtifact {
  return {
    topic,
    sources: [
      { id: "s1", title: "Federal Reserve Economic Data (FRED)", url: "https://fred.stlouisfed.org", excerpt: "Historical economic data including GDP, inflation, and unemployment rates." },
      { id: "s2", title: "Bureau of Labor Statistics", url: "https://www.bls.gov", excerpt: "Official employment and price statistics for the United States." },
      { id: "s3", title: "International Monetary Fund Reports", url: "https://www.imf.org", excerpt: "Global economic outlook and country-level financial assessments." },
    ],
    claims: [
      { id: "c1", claim: `${topic} had measurable impact on employment and GDP growth`, sourceIds: ["s1", "s2"], confidence: "high" },
      { id: "c2", claim: "Central bank policy responses were rapid and unprecedented", sourceIds: ["s1", "s3"], confidence: "high" },
      { id: "c3", claim: "Recovery timelines varied significantly across regions", sourceIds: ["s3"], confidence: "medium" },
    ],
    allowedFacts: [
...
function generateMockScript(topic: string, theme: string, research: ResearchArtifact): ScriptArtifact {
    ...
    scenes: [
      {
        ...
        narrationSegment: "",
        title: title.split(" ").slice(0, 6).join(" "),
        data: { title: title.split(" ").slice(0, 6).join(" "), subtitle: "A Financial Documentary" },
        needsImage: false,
        narrativeRole: "intro",
      },
      {
        ...
        data: {
          ...
          context: "Markets plunged and unemployment soared as the global financial system froze.",
        },
        needsImage: true,
        imagePrompt: "Dramatic financial district skyline at dusk, empty trading floor with red screens showing falling numbers, cinematic documentary photography, moody lighting",
        narrativeRole: "hook",
      },
      {
        id: "scene3",
        componentSlug: "bar-chart",
        narrationSegment: "But what actually happened? The crisis began with a simple idea: that housing prices would always rise. Banks packaged risky mortgages into complex securities, rating agencies stamped them as safe, and investors bought them by the billions.",
      ...
      },
      {
        ...
        narrationSegment: "When the housing bubble burst, the entire financial system froze. Banks stopped lending. Businesses couldn't make payroll.",
      ...
      },
      {
        ...
        narrationSegment: "The government responded with the largest bailout in history, injecting hundreds of billions into the financial system.",
        title: "Emergency Bailout as % of GDP",
        data: {
          title: "Emergency Bailout as % of GDP",
          percentage: 25,
          label: "The bailout package equaled roughly a quarter of annual economic output.",
          sublabel: "of GDP",
        },
        needsImage: false,
        narrativeRole: "fact",
      },
      {
        ...
        narrationSegment: "The recovery was slow and uneven. Some regions bounced back within two years. Others took nearly a decade. The crisis exposed deep flaws in how we regulate finance, and it changed the way central banks think about risk forever.",
        title: "Lessons for the Future",
        data: {
          conclusion: "The crisis changed financial regulation forever.",
          takeaway: "It exposed deep flaws in how we regulate finance and changed the way central banks think about risk. The question is whether we've learned enough to prevent the next one.",
...
function generatePlaceholderPng(width: number, height: number): Buffer {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      rawData[px] = 40; rawData[px + 1] = 40; rawData[px + 2] = 50;
    }
  }
...
export async function run(): Promise<SpikeResult> {
  ...
  // ─── Stage 7: Composition ──────────────────────────────────────────────────
  const composition = await runComposition(script, actualScenePlan, media, musicSync, theme, outDir);
  ...
  try {
    ...
    console.error(`\n✗ Render failed: ${err}`);
    log("Render", "Render step failed — composition artifacts are still available for manual rendering");
  }
