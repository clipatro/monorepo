...
export const BarChart: React.FC<Props> = ({ data, theme, delay = 5 }) => {
	...
	return (
		<DocumentaryCanvas
			theme={theme}
			delay={delay}
			eyebrow="Comparative measure"
			footer={data.yAxisLabel}
			contentStyle={{ justifyContent: "flex-start", paddingTop: 20 }}
		>
			<DocumentaryReveal delay={delay + 4} direction="wipe">
				<h1
					style={{
						margin: 0,
						maxWidth: 590,
						fontFamily: t.display,
						fontSize: data.title.length > 55 ? 68 : 82,
						lineHeight: 0.9,
			...
			<div style={{ display: "grid", gap: 14, marginTop: 36 }}>
				{bars.map((bar, index) => {
					const color =
						bar.color ?? (index === bars.length - 1 ? t.accent : t.bright);
					const progress = interpolate(
						frame,
						[delay + 14 + index * 8, delay + 50 + index * 8],
						[0, bar.value / max],
						{
					...
					const valueProgress = interpolate(
						frame,
						[delay + 18 + index * 8, delay + 52 + index * 8],
						[0, 1],
					...
					return (
						<div key={`${bar.label}-${index}`}>
							...
							>
								...
								>
									{(bar.value * valueProgress).toFixed(
										Number.isInteger(bar.value) ? 0 : 1,
									)}
								</span>
							</div>
							<div
