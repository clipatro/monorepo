...
export const KeyFact: React.FC<TemplateProps<KeyFactData>> = ({ data, theme, delay = 0 }) => {
  ...
  return (
    <DocumentaryCanvas theme={theme} delay={delay} eyebrow={data.label ?? "Key fact"} footer={data.source} variant="paper">
      ...
      {data.detail ? <DocumentaryReveal delay={delay + 30} direction="left" style={{ marginTop: 42, width: 520 }}><div style={{ color: `${t.base}bb`, fontFamily: t.serif, fontSize: 27, lineHeight: 1.4 }}>{data.detail}</div></DocumentaryReveal> : null}
    </DocumentaryCanvas>
  );
};
...
export const StatisticSpotlight: React.FC<TemplateProps<StatisticSpotlightData>> = ({ data, theme, delay = 0 }) => {
  ...
  return (
    <DocumentaryCanvas theme={theme} delay={delay} eyebrow={data.label} footer={data.source ?? "Measured impact"} contentStyle={{ justifyContent: "flex-end", paddingBottom: 110 }}>
...
export const ComparisonSplit: React.FC<TemplateProps<ComparisonSplitData>> = ({ data, theme, delay = 0 }) => {
  ...
  return (
    <DocumentaryCanvas theme={theme} delay={delay} eyebrow={data.title} footer={data.verdict}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 44 }}>
        {sides.map((side, index) => <DocumentaryReveal key={side.label} delay={delay + 14 + index * 14} direction={index === 0 ? "left" : "right"}>
          <div style={{ minHeight: 460, display: "flex", flexDirection: "column", justifyContent: "space-between", alignItems: index === 0 ? "flex-start" : "flex-end", textAlign: index === 0 ? "left" : "right" }}>
            <div style={{ color: side.color, fontFamily: t.mono, fontSize: 15, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase" }}>{side.label}</div>
  ...
  return (
    <DocumentaryCanvas theme={theme} delay={delay} eyebrow={data.title} icon="time" footer={data.change ?? "Change over time"} variant="paper">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 38, color: t.base }}>
        {[data.before, data.after].map((item, index) => <DocumentaryReveal key={index} delay={delay + 8 + index * 28} direction={index === 0 ? "left" : "right"}><div style={{ minHeight: 480, display: "flex", flexDirection: "column", justifyContent: index === 0 ? "flex-start" : "flex-end", paddingBottom: index === 0 ? 0 : 20 }}><DocumentaryPill theme={theme} color={index === 0 ? `${t.base}88` : t.accent}>{item.label ?? (index === 0 ? "Before" : "After")}</DocumentaryPill><div style={{ marginTop: 28, color: index === 0 ? `${t.base}99` : t.accent, fontFamily: t.display, fontSize: 104, lineHeight: 0.85 }}>{item.value}</div><div style={{ marginTop: 20, color: `${t.base}aa`, fontFamily: t.serif, fontSize: 23, lineHeight: 1.4 }}>{item.detail}</div></div></DocumentaryReveal>)}
      </div>
    </DocumentaryCanvas>
  );
};
